...
function tabChangeText() {
  getMarketPanel().should((item) => {
    ...
    const haRows = item.find(dataTestId).length >= 1;
    void expect(isEmpty || haRows).to.be.true;
  });
}
...
describe('로그인 후 마켓 컴포넌트에서 발생하는 흐름', () => {
  ...
  describe('필터링', () => {
    it('원화·관심 탭에서 현재가·전일대비·거래대금 헤더 누르면 정렬된다', () => {
      ...
      getMarketItemList()
        .find(dataTestId)
        .then((rows) => rows.toArray().map((row) => row.innerText))
        .as('orderBefore');
  ...
  describe('리스트 아이템/상세 이동', () => {
    it('리스트에서 종목 클릭하면 차트와 호가창이 보인다', () => {
      ...
      cy.get('[data-testid="chart-tab"]').should('be.visible');

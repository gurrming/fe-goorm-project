...
export default function NotificationItem({ item }: { item: TNotification }) {
    ...
    const days = hours / 24;
    const time = days > 1 ? `${Math.floor(days)}일 전` : hours > 1 ? `${Math.floor(hours)}시간 전` : minutes > 1 ? `${Math.floor(minutes)}분 전` : `${Math.floor(seconds)}초 전`;
    return (
        <div key={item.notificationId} className={`${item.notificationIsRead ? 'bg-gray-100' : 'bg-white'} p-4 flex flex-col gap-1 hover:bg-gray-100`}>

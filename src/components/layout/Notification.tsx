...
export default function Notification({ anchorRect, width = '400px', setOpen }: { anchorRect: DOMRectReadOnly, width: string, setOpen: (open: boolean) => void }) {
    ...
    const positionStyle = anchorRect
        ? {
            ...
            left: Math.max(
            8,
            Math.min(window.innerWidth - parsedWidth - 8, anchorRect.right - parsedWidth),
            ),

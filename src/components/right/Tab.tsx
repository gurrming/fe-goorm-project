const Tab = ({ handleTab, tab }: { handleTab: (tab: string) => void; tab: string }) => {
  return (
    <div className="flex gap-2 justify-between items-center border-b border-gray-200 text-[#333333]">
      <p
        className={`w-full text-sm font-bold text-center py-3 ${tab === 'buy' ? 'text-[#DD3C44]] border-b-[3px] border-[#DD3C44]' : ''}`}
        onClick={() => handleTab('buy')}
      >
        매수
      </p>
      <p
        className={`w-full text-sm font-bold text-center py-3 ${tab === 'sell' ? 'text-[#0062DF]] border-b-[3px] border-[#0062DF]' : ''}`}

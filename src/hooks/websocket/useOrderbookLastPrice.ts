...
/**
 * /topic/orderbook/lastPrice 구독 훅
 * 호가 테이블에 반영되는 체결가(검정 테두리) 데이터를 받아옵니다.
 * 받아온 데이터는 zustand store에 저장됩니다.
 */
export const useOrderbookLastPrice = () => {
  const { isConnected, stompClientRef } = useWebsocket();
  const { setLastPrice } = useOrderbookStore();

  useEffect(() => {
    if (isConnected && stompClientRef.current) {
      const subscription = stompClientRef.current.subscribe('/topic/orderbook/lastPrice', (message) => {
        try {
